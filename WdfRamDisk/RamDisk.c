...
#pragma alloc_text(PAGE, rdEvtCleanup)
#pragma alloc_text(PAGE, rdQueryDiskParameter)
#pragma alloc_text(PAGE, rdFormatDisk)
#endif //ALLOC_PRAGMA
...
)
{
    ...
    PULONG              firstFatSector; //FAT表起始扇区
    PDIR_ENTRY          rootDir;        //根目录所在扇区

    PAGED_CODE();

    static_assert(sizeof(BOOT_SECTOR_FAT32) == DEFAULT_SECTOR_SIZE, "error BOOT_SECTOR_FAT32 size!");
    static_assert(sizeof(FSINFO_SECTOR) == DEFAULT_SECTOR_SIZE, "error FSINFO_SECTOR size!");
    ...
    DevExt->DiskGeometry.TracksPerCylinder  = 2; //每个柱面磁道数
    DevExt->DiskGeometry.MediaType          = RAMDISK_MEDIA_TYPE;
    DevExt->DiskGeometry.Cylinders.QuadPart = DevExt->DiskRegInfo.DiskSize / (DEFAULT_SECTOR_SIZE * 32 * 2); //柱面数
    ...
    bootSector->bsOemName[0] = 'M';
    bootSector->bsOemName[1] = 'S';
    bootSector->bsOemName[2] = 'D';
    bootSector->bsOemName[3] = 'O';
    bootSector->bsOemName[4] = 'S';
    bootSector->bsOemName[5] = '5';
    ...
    fsInfoSector->bsSig2[2] = 0xAA;

    //初始化文件目录表    
    firstFatSector = (PULONG)(bootSector + bootSector->bsResSectors);
    RtlZeroMemory(firstFatSector, bootSector->bsSecPerFAT * DevExt->DiskGeometry.BytesPerSector);

    firstFatSector[0] = 0x0FFFFFF8;
    firstFatSector[1] = 0x0FFFFFFF;
    firstFatSector[2] = 0x0FFFFFFF;

    //初始化根目录
    rootDir = (PDIR_ENTRY)(bootSector + bootSector->bsResSectors + bootSector->bsFATs * bootSector->bsSecPerFAT);
    RtlZeroMemory(rootDir, bootSector->bsSecPerClus * DevExt->DiskGeometry.BytesPerSector);
